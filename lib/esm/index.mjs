...
export class AuthorizedPocketBase {
    ...
    async authorizeWithAdminEmailAndPassword(email, password) {
    ...
    async authorize() {
        ...
        switch (authMethod) {
            case "admin-email":
                await this.authorizeWithAdminEmailAndPassword(this._getAdminEmailVars().PB_USERNAME, this._getAdminEmailVars().PB_PASSWORD);
                break;
            default:
                throw new Error(`No authorization method selected \n 
    ...
    _getAdminEmailVars = () => {
        if (import.meta.env.PB_USERNAME == undefined ||
